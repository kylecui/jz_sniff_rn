                                       ...
                                       const char names[][32], int count)
{
    ...
    if (sock < 0) {
        jz_log_error("prepare_monitor: socket(): %s", strerror(errno));
        return;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        (void)snprintf(ifr.ifr_name, IFNAMSIZ, "%s", names[i]);

        if (ioctl(sock, SIOCGIFFLAGS, &ifr) < 0) {
            jz_log_error("prepare_monitor: SIOCGIFFLAGS(%s): %s",
                          names[i], strerror(errno));
            continue;
        }

        short old_flags = ifr.ifr_flags;
        ifr.ifr_flags |= IFF_UP | IFF_PROMISC;

        if (ifr.ifr_flags == old_flags) {
            jz_log_info("prepare_monitor: %s already UP+PROMISC", names[i]);
            continue;
        }

        if (ioctl(sock, SIOCSIFFLAGS, &ifr) < 0) {
            jz_log_error("prepare_monitor: SIOCSIFFLAGS(%s): %s",
                          names[i], strerror(errno));
            continue;
        }

        jz_log_info("prepare_monitor: %s flags 0x%04x -> 0x%04x (UP+PROMISC)",
                      names[i], (unsigned)old_flags, (unsigned)ifr.ifr_flags);
    }
...

    {
        FILE *fp;
        int wrote = 0;
        const char *np_path = "/etc/netplan/90-jz-monitors.yaml";

        fp = fopen(np_path, "w");
        if (!fp) {
            jz_log_warn("prepare_monitor: cannot write %s: %s",
                          np_path, strerror(errno));
            goto done;
        }

        fprintf(fp, "# Auto-generated by sniffd — do not edit\n");
        fprintf(fp, "network:\n");
        fprintf(fp, "  version: 2\n");
        fprintf(fp, "  ethernets:\n");

        for (i = 0; i < cfg->system.interface_count &&
            ...
            if (strcmp(iface->address, "dhcp") == 0) {
                ...
                fprintf(fp, "      activation-mode: manual\n");
            }
        ...
        fclose(fp);
        (void)chmod(np_path, 0600);

        if (wrote > 0) {
            jz_log_info("prepare_monitor: wrote %s (%d interfaces)",
                          np_path, wrote);
            if (system("netplan apply 2>/dev/null") != 0)
                jz_log_warn("prepare_monitor: netplan apply failed (non-fatal)");
        } else {
            (void)unlink(np_path);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    prepare_monitor_interfaces(&g_ctx.config, xdp_names, xdp_iface_count);
